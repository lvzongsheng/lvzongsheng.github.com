...
关于上述题型的总结，可以参见[二叉树的遍历（递归、非递归）分析](http://biaobiaoqi.me/blog/2013/04/27/travsal-binary-tree/)和[根据前中后序和层序重建二叉树](http://biaobiaoqi.me/blog/2013/04/27/pat1020-pat1043-rebuild-binary-tree/)<br/>这里总结一下二叉树遍历最一般使用的STL。
在前中后序遍历中，最为常用的还是递归的实现，一般会使用stack或者queue。但在层序遍历中，我们使用的queue，而stack不能实现层序遍历。
